...
import { DefaultSudoCryptoProvider } from './sudoCryptoProvider'
global.crypto = require('isomorphic-webcrypto')
global.btoa = (b) => Buffer.from(b).toString('base64')
global.atob = (a) => Buffer.from(a, 'base64').toString()

const cryptoProvider = new DefaultSudoCryptoProvider()
...
describe('sudoCryptoProvider', () => {
  ...
  describe('encryptWithSymmetricKey', () => {
    it('should throw KeyNotFoundError when symmetric key not set', async () => {
      await expect(
        cryptoProvider.encryptWithSymmetricKey(
          'random.symmetric',
    ...
    it('should encrypt then decrypt', async () => {
      ...
      const encryptedBuffer = await cryptoProvider.encryptWithSymmetricKey(
        'testKey.symmetric',
        new TextEncoder().encode('data to encrypt'),
      )
