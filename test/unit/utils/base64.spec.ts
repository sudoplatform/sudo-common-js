...
describe('Base64', () => {
  ...
  it('Base 64 encodes and decodes its own results', () => {
    const encoded = Base64.encode(BufferUtil.fromString('dummy_data'))
    const decoded = Base64.decode(encoded)
    expect(BufferUtil.toString(decoded)).toBe('dummy_data')
  })

  it('Base 64 encodes the same as other encoders', () => {
    expect(Base64.encode(BufferUtil.fromString('dummy_data'))).toEqual(
      dummyDataB64,
    )
  })
  ...
  it('Base 64 URL safe encodes an ArrayBuffer, and decodes to an ArrayBuffer', () => {
    ...
    const encoded = Base64.urlSafeEncode({
      input: BufferUtil.fromString(result),
    })
    expect(Base64.UrlSafeValidate(encoded)).toStrictEqual(true)
    const decoded = Base64.urlSafeDecode({ encoded })
    expect(BufferUtil.toString(decoded)).toStrictEqual(result)
  })
  ...
  it('should encode like buffer encode/decode', () => {
    ...
    const decodedPKBuffer = Base64.decode(encodedPK)
